...
void PuzzleWidget::onUpdatePixmap()
{
    ...
    // 计算有效的范围
    int maxX = qMax(1, width() - squarewidth - squareradius);  // 确保最小值为1
    int maxY = qMax(1, height() - squarewidth - squareradius); // 确保最小值为1
    
    // 将可用区域划分为多个部分，确保拼图块可以出现在不同位置
    int xSection = qMax(1, maxX / 3);  // 确保不为0
    int ySection = qMax(1, maxY / 2);  // 确保不为0
    
    // 使用 QRandomGenerator 生成随机数（确保范围大于0）
    int section = QRandomGenerator::global()->bounded(1, 7); // 1-6的范围
    
    // 根据选中的区域计算基准位置
    int baseX = ((section - 1) % 3) * xSection;
    int baseY = ((section - 1) / 3) * ySection;
    
    // 在选中的区域内随机生成具体位置，确保范围至少为1
    int randX = QRandomGenerator::global()->bounded(1, xSection + 1);
    int randY = QRandomGenerator::global()->bounded(1, ySection + 1);
    
    m_offsetPoint.rx() = qBound(0, baseX + randX, maxX);
    m_offsetPoint.ry() = qBound(0, baseY + randY, maxY);
