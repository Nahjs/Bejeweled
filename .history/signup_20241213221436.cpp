    ...
    useSliderVerification(false)
{
    ...
    // 在验证码输入框上方插入选择框
    QLayoutItem* item = ui->lineEdit_verification->parentWidget()->layout();
    if (QVBoxLayout* vLayout = qobject_cast<QVBoxLayout*>(item)) {
        vLayout->insertLayout(vLayout->indexOf(ui->lineEdit_verification), methodLayout);
    }
    ...
    // 创建验证框架并设置布局
    ui->verificationFrame = new QFrame(this);
    ...
    // 将验证框架添加到现有布局中
    if (QVBoxLayout* vLayout = qobject_cast<QVBoxLayout*>(ui->lineEdit_verification->parentWidget()->layout())) {
        vLayout->insertWidget(vLayout->indexOf(ui->lineEdit_verification) + 1, ui->verificationFrame);
    }
